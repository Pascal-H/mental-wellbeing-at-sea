    ...
    for p_idx in range(n_participants):
        ...
        session_dates = sorted(
            [
                ...
                for d in rng.integers(0, 60, size=n_sessions)
            ]
        )
...
def main():
    ...
    if sf is not None:
        ...
        for i, (rel_path, duration_s) in enumerate(audio_specs):
            ...
            waveform = generate_synthetic_audio(duration_s, sr=args.audio_sr, rng=rng)
    ...
    test_speakers = rng.choice(unique_participants, size=n_test, replace=False).tolist()
    ...
    print(
        f"  python src/main.py {os.path.relpath(config_path, os.path.dirname(os.path.dirname(script_dir)))}"
    )
    print()
    print(
