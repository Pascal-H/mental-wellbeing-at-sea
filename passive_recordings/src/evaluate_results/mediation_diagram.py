...
def _coef_color(coef: float) -> str:
    ...
    return _POS_COLOR if coef > 0 else _NEG_COLOR
...
def create_mediation_diagram(
    ...
    # Switch to PGF backend temporarily
    ...
    _draw_rect(ax, *m_pos, bw_m, bh, "Emotion Intensity (M)", fontsize=fs_var, lw=lw)
    _draw_rect(ax, *y_pos, bw_y, bh, "Self-Reported Stress (Y)", fontsize=fs_var, lw=lw)
    ...
    legend_handles = [
        ...
        Line2D([0], [0], color="gray", lw=lw, ls="--", label=r"Direct effect ($c'$)"),
    ]
    if show_confounders:
        legend_handles.append(
            Line2D(
                [0], [0], color=_CONF_COLOR, lw=0.5, ls="--", label="Control variables"
            )
        )
    ax.legend(
