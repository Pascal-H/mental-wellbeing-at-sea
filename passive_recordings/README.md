...
The evaluation scripts can be run end-to-end using synthetic data generated by
`synthetic_data/passive/generate_synthetic_data.py` (see
[`synthetic_data/README.md`](../synthetic_data/README.md)). In the current
state, hardcoded paths in the following files are set to point to the synthetic
data directories (original paths are preserved as comments):

- `src/evaluate_results/evaluate_time_course_args_external.py`
- `src/evaluate_results/confounder_noise_denoising_main.py`
- `src/evaluate_results/mediation_analysis_main.py`
...
The `--quick` flag reduces bootstrap iterations to 10 for fast verification.
Synthetic results are written to `synthetic_data/passive/data/evaluated/` so
that no original results under `data/evaluated/` are overwritten. The upstream
audio compression (`compress_extracted_files/`) and prediction
(`process_and_predict/`) scripts require real VDR audio and additional
dependencies and are not covered by the synthetic data pipeline.
